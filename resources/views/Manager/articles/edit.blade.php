...
@section('content')
    ...
    <div class="container">
        <div class="card card-custom gutter-b example example-compact">
            ...
            <!--begin::Form-->
            <form method="POST" class="form-article-edit" action="{{route('articles.update', $article->id)}}"
                ...
                <div class="card-body">
                    <div class="form-group">
                        ...
                        <input type="text" class="form-control @error('title') is-invalid @enderror"
                               id="article_title" name="title" value="{{old('title', $article->title)}}">
                        @error('title')
                    ...
                    <div class="form-group ">
                        ...
                        <input type="text"
                               ...
                               id="article_heading" name="heading" value="{{old('heading', $article->heading)}}">
                        @error('heading')
                        <div class="alert alert-danger">{{ $message }}</div>
                        @enderror
                    </div>
                    <div class="form-group">
                        ...
                        <select title="Hãy chọn danh mục bài viết" name="category_ids[]" id="article_category"
                            ...
                            @foreach($article_categories as $key => $category)
                                <option
                                    value="{{$category->id}}"
                                    @if(in_array($category->id, old('category_ids', $category_ids))) selected @endif>{{$category->name}}</option>
                            @endforeach
                        </select>
                    </div>
                    <div class="form-group">
                        <img src="{{$article->image}}" class="img-fluid" alt="" width="50%">
                    </div>
                    <div class="form-group">
                    ...
                    <div class="form-group">
                        ...
                        <input id="kt_tagify_1" class="form-control tagify @error('tags') is-invalid @enderror"
                               value="{{old('tags', implode(", ", $tags))}}" name="tags" placeholder="type..."
                               autofocus="">
                        @error('tags')
                        <div class="alert alert-danger">{{ $message }}</div>
                        @enderror
